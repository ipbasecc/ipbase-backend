...
module.exports = createCoreService('api::todo.todo',({strapi}) => ({
    ...
    process_updateTodo_params(...args){
        ...
        if(data.hasOwnProperty('status') && (isFeedbackOwner ||  (fields_permission.includes('status') || belonged_user))) {
        ...
        if(data.color_marker && (isFeedbackOwner ||  (fields_permission.includes('color_marker') || belonged_user))) {
        ...
        if(data.importance && (isFeedbackOwner ||  (fields_permission.includes('importance') || belonged_user))) {
        ...
        if(data.urgency && (isFeedbackOwner ||  (fields_permission.includes('urgency') || belonged_user))) {
        ...
        if(data.attachment && (isFeedbackOwner || (fields_permission.includes('content') || belonged_user))) {
            params.attachment = {
                set: data.attachment
            }
        } else if (data.attachment && !belonged_user && !fields_permission.includes('content')) {
            ctx.throw(401, '您无权修改该待办附件')
        }
